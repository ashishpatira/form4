...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # Identify first boundary breach (price dropping to 0 or below)
    is_negative = multipliers <= 0
    if is_negative.any():
        first_negative_idx = np.argmax(is_negative)
        # Zero out this and all subsequent multipliers to emulate absorbing barrier at 0
        multipliers[first_negative_idx:] = 0

    prices = np.zeros(days + 1)
    prices[0] = initial_price
    # Vectorized computation of prices using cumprod
