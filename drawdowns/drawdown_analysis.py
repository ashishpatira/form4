...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # ⚡ Bolt optimization: Vectorized price simulation replacing the O(N) loop
    # We calculate daily return multipliers and use cumulative product (cumprod).
    multipliers = (drift / 252 + 1 + noise)

    # Handle absorbing barrier at 0. If a multiplier drops <= 0, the price hits 0.
    # To maintain mathematical correctness with cumprod, we zero out all subsequent multipliers.
