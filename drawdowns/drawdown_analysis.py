...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # Vectorized computation of multipliers
    multipliers = drift / 252 + 1 + noise

    # Preallocate output array and set the initial price
    prices = np.empty(days + 1)
    prices[0] = initial_price

    # Vectorized simulation using np.cumprod instead of python for loop
    prices[1:] = initial_price * np.cumprod(multipliers)

    # Ensure price doesn't go negative at the end
