...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # Instead of calculating step-by-step in a slow Python loop, we construct an array
    # of multipliers for each day. The cumulative product of these multipliers (np.cumprod)
    # efficiently computes the entire time series.
    # The first element multiplier is 1.0 (to maintain initial_price).
    multipliers = np.concatenate(([1.0], drift / 252 + 1 + noise))
    prices = initial_price * np.cumprod(multipliers)

    # Vectorized check to ensure price doesn't go negative
