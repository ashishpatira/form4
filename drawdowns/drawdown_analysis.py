...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # Vectorized calculation of daily price multipliers
    multipliers = drift/252 + 1 + noise

    # Enforce absorbing barrier at 0:
    # If any multiplier is <= 0, the price drops to <= 0 and should stay 0.
    is_negative = multipliers <= 0
    if is_negative.any():
        first_negative_idx = is_negative.argmax()
