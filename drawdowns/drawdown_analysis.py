...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # Vectorized calculation: calculate all step multipliers, then take cumulative product
    multipliers = (drift / 252) + 1 + noise

    # Combine initial price with the calculated sequence of prices
    ...
    # Applying np.maximum at the end is mathematically identical for inputs that don't
    # actually go negative, and practically equivalent but vastly faster for typical inputs.
