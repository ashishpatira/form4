...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # ⚡ Bolt Optimization: Replace slow Python for-loop with vectorized NumPy operations.
    # Calculate day-over-day multipliers
    multipliers = (drift / 252) + 1 + noise

    # Calculate cumulative product of multipliers
    cumulative_returns = np.cumprod(multipliers)

    # Prepend initial price and apply cumulative returns
    prices = np.concatenate(([initial_price], initial_price * cumulative_returns))

    # Ensure price doesn't go negative (vectorized equivalent of floor)
