...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # ⚡ Bolt Optimization: Replaced Python for-loop with vectorized NumPy operations
    # calculating cumulative product (np.cumprod) over the returns.
    returns = np.concatenate(([1.0], drift/252 + 1 + noise))
