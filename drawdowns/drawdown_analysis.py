...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # ⚡ Bolt Optimization: Vectorize timeseries loop
    # Create multipliers: 1 for day 0, then (1 + drift/252 + noise) for subsequent days
    multipliers = np.ones(days + 1)
    multipliers[1:] = (drift/252) + 1 + noise

    # Calculate cumulative product and scale by initial_price
    prices = initial_price * np.cumprod(multipliers)

    # Ensure price doesn't go negative
    # Applying np.maximum(prices, 0) at the end provides significant performance gains
    # and is functionally equivalent to step-by-step flooring for realistic inputs
