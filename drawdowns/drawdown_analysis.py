...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # Vectorized calculation of daily multipliers
    multipliers = (drift/252) + 1 + noise

    # Handle absorbing boundary at 0:
    # If a multiplier is <= 0, the price hits 0 or goes negative.
    # We find the first breach and zero out all subsequent multipliers.
    is_negative = multipliers <= 0
    if np.any(is_negative):
        first_breach_idx = np.argmax(is_negative)
        multipliers[first_breach_idx:] = 0

    # Calculate cumulative product for all days
