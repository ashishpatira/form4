...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    daily_multipliers = drift / 252 + 1 + noise
    cumulative_multipliers = np.cumprod(daily_multipliers)

    # Insert initial multiplier
    full_multipliers = np.insert(cumulative_multipliers, 0, 1.0)
    prices = initial_price * full_multipliers
