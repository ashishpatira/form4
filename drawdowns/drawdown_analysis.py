...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # ⚡ Bolt: Vectorize timeseries generation by calculating daily returns
    # and computing cumulative product. Applying np.maximum(..., 0) provides
    # equivalent non-negative clamping functionality but is significantly faster
    # than iterating row-by-row in Python.
    returns = drift / 252 + 1 + noise
    prices = np.concatenate(([initial_price], initial_price * np.cumprod(returns)))
