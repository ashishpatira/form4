...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # Vectorized calculation: pre-calculate daily multipliers
    multipliers = drift / 252 + 1 + noise

    # Prepend 1.0 to include initial_price calculation correctly
    # Use np.cumprod to get the cumulative multiplier over time
    cum_returns = np.cumprod(np.insert(multipliers, 0, 1.0))
    prices = initial_price * cum_returns

    # Ensure price doesn't go negative, though highly unlikely with this drift and vol
    # np.maximum operates on the whole array and is much faster than checking step-by-step
