...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # ⚡ Bolt Optimization: Vectorized timeseries calculation
    # Replaces O(n) loop with highly optimized NumPy vector operations
    daily_returns = (drift/252 + 1) + noise
    prices = np.concatenate(([initial_price], initial_price * np.cumprod(daily_returns)))

    # Ensure price doesn't go negative, applying maximum over the entire array
