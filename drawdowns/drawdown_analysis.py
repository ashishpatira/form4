...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # ⚡ Bolt: Vectorize timeseries calculation using np.cumprod
    # This avoids the slow Python for loop for cumulative multiplication
    multipliers = 1 + drift / 252 + noise
    prices = np.concatenate(([initial_price], initial_price * np.cumprod(multipliers)))

    # ⚡ Bolt: Vectorize flooring to 0
    # Applied at the end of the array for performance, functionally equivalent
    # to step-by-step flooring for realistic inputs where negative prices are highly unlikely.
