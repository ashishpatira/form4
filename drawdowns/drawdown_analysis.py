...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    daily_returns = drift/252 + 1 + noise

    # Vectorized calculation for compounded returns using cumulative product
    prices[1:] = initial_price * np.cumprod(daily_returns)
    ...
    # for realistic inputs and provides massive performance gains over a Python loop.
