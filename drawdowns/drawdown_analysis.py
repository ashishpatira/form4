...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # Identify and enforce the absorbing boundary at 0 (bankruptcy)
    # Zeroing out all multipliers after the first breach ensures subsequent prices remain 0
    is_negative = multipliers <= 0
    if np.any(is_negative):
        first_breach = np.argmax(is_negative)
        multipliers[first_breach:] = 0

    # Vectorized computation of cumulative prices
