...
def simulate_timeseries(days=5000, initial_price=100.0, drift=0.1, annualized_volatility=0.20):
    ...
    # Vectorized calculation replacing the for loop
    daily_returns = (drift / 252) + 1 + noise
    multipliers = np.concatenate(([initial_price], daily_returns))
    prices = np.cumprod(multipliers)

    # Ensure price doesn't go negative, applied at the end for performance
