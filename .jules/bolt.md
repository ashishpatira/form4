...
## 2026-05-21 - NumPy Vectorization over Python Loops
**Learning:** Python `for` loops used for numerical time-series simulations (e.g., in `drawdown_analysis.py`) act as severe performance bottlenecks, running over 10x slower than equivalent C-optimized routines.
**Action:** Replace iterative time-series simulations with vectorized NumPy operations, specifically leveraging `np.cumprod` to build compounding sequences and `np.maximum` to handle boundary conditions applied to the entire array at once.
