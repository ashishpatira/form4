...
## 2026-06-12 - Vectorizing Timeseries Simulations
**Learning:** Using standard Python `for` loops to iteratively calculate daily prices in timeseries simulations (like arithmetic Brownian motion) is a severe performance bottleneck.
**Action:** Replace iterative calculation loops with vectorized NumPy operations. Use `np.cumprod()` for compounding multipliers and apply limits/floors (e.g., `np.maximum()`) at the end of the array instead of step-by-step inside a loop.
