...
## 2026-05-25 - Python Loops vs Vectorized Operations Bottleneck
...
**Action:** Replace them with vectorized NumPy operations, specifically leveraging `np.cumprod` and `np.maximum`. When replacing iterative loops with vectorized NumPy operations, ensure that remnants of the loop initialization (such as pre-allocating arrays with `np.zeros`) are removed as they become dead code.
