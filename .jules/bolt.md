...
**Learning:** Standard Python `for` loops in numerical arrays act as severe performance bottlenecks, particularly for time series simulations where each step depends on the previous.
**Action:** Replace `for` loops with vectorized NumPy operations, leveraging `np.cumprod` and `np.maximum`.
