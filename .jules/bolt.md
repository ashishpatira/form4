...
## 2024-05-18 - Vectorizing Time Series
**Learning:** Python `for` loops act as a severe bottleneck for time series simulation arrays. The step-by-step conditional bounds checks logic can be effectively vectorized and run significantly faster.
**Action:** Replace sequential iterative updates over time series arrays with `np.cumprod(np.insert(returns, 0, 1.0))` and evaluate bounds once at the end using `np.maximum(prices, 0)`.
