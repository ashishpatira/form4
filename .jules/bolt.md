...
## 2023-10-27 - Vectorizing Time Series Simulation
**Learning:** Standard Python `for` loops act as severe performance bottlenecks in numerical array generation and time series simulation.
**Action:** Replace iterative calculation loops with vectorized NumPy operations (e.g., `np.cumprod` and `np.maximum`) to achieve significant performance gains (e.g. 3-5x speedups for large series).
