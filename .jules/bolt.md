...
## 2026-04-03 - Vectorize timeseries loops with np.cumprod
**Learning:** Python `for` loops in numerical processing and financial timeseries generation (like applying sequential drift/volatility multipliers) act as severe performance bottlenecks.
**Action:** Always replace standard iterative `for` loops in time series simulation and array generation with vectorized NumPy operations, specifically leveraging `np.cumprod` for sequential multiplication and applying array-wide limits (like `np.maximum(prices, 0)`) at the end for maximal performance.
