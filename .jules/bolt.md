...
## 2026-04-03 - Vectorized Time Series Bottlenecks
**Learning:** Standard Python `for` loops used for step-by-step sequential operations on numerical arrays (such as calculating Brownian motion paths) act as a severe performance bottleneck.
**Action:** Replace iterative time series loops with vectorized NumPy operations, specifically leveraging `np.cumprod` and conditionally `np.maximum(prices, 0)` at the end to significantly accelerate processing (from O(N) in Python execution to optimized C-level speed).
