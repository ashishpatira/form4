...
## 2024-05-18 - Avoid loops in time series simulation
**Learning:** For simulations of time series data involving paths built via iterative formulas (e.g., Brownian motion), Python `for` loops are severe performance bottlenecks.
**Action:** Always replace Python loops with vectorized NumPy operations, prioritizing `np.cumprod` and applying safety measures like `np.maximum` at the end instead of conditional checks inside a loop. This can provide >10x speedup.
