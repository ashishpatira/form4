...
## 2026-04-03 - Vectorizing Python Loops for Numerical Arrays
**Learning:** Standard Python `for` loops act as severe performance bottlenecks when used for time series simulation and large numerical array manipulations. In this codebase, simulating a million days took ~0.9s with a loop.
**Action:** Replace `for` loops in numerical calculations with vectorized NumPy operations whenever possible. Specifically, use functions like `np.cumprod` and `np.maximum` to eliminate the loop overhead and achieve ~10x speedups.
