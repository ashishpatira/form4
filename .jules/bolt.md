...
## 2025-05-17 - drawdowns/drawdown_analysis.py vectorization
**Learning:** Pure Python `for` loops used for building sequential timeseries (e.g. arithmetic Brownian motion simulations with daily drift and volatility) are a major performance bottleneck compared to native numpy C-implementations.
**Action:** Replace sequential time series generation loops with vectorized NumPy equivalents, such as `np.cumprod(multipliers)` combined with `np.maximum(prices, 0)`, to instantly apply compounding returns and clamps.
