...
## 2026-05-01 - Python for Loop Bottlenecks in Time Series
**Learning:** Standard Python `for` loops used for numerical arrays and time series simulation act as a severe performance bottleneck compared to NumPy's optimized C backend.
**Action:** Always replace iterative standard Python loops processing time series data with vectorized NumPy operations, specifically leveraging `np.cumprod` and `np.maximum`, which provided over a 70% execution time reduction.
