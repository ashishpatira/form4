...
## 2026-04-03 - NumPy Vectorization over Python loops for simulation
**Learning:** Python `for` loops act as severe performance bottlenecks for time series simulation over numerous numerical arrays compared to applying mathematical operations across whole arrays.
**Action:** Replace sequential operations over large datasets with vectorized `NumPy` methods like `np.cumprod` and `np.maximum`.
