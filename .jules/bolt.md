...
## 2026-04-03 - Vectorizing Timeseries with Absorbing Bounds
**Learning:** When vectorizing timeseries simulation loops that enforce absorbing bounds (like flooring prices to 0 upon bankruptcy), applying a final `np.maximum` after `np.cumprod` introduces a mathematical regression, because the bound wasn't treated as absorbing during intermediate steps.
**Action:** Identify the first boundary breach (e.g., `is_negative = multipliers <= 0`), find its index, and zero out all subsequent multipliers before running `np.cumprod` to ensure correctness while maintaining the massive performance gains of vectorization.
