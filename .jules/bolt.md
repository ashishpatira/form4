...
**Learning:** Python `for` loops act as severe performance bottlenecks when simulating large time series (e.g., thousands of steps). Using standard loops to calculate step-by-step price updates based on random noise is inefficient.
**Action:** Replace `for` loops in numerical arrays and time series simulations with vectorized NumPy operations, specifically leveraging `np.cumprod` for cumulative products and `np.maximum` to enforce boundaries (e.g. preventing negative prices).
