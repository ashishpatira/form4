...
## 2026-04-03 - Vectorizing Time Series Loops
**Learning:** Python `for` loops used for building iterative time series (like geometric random walks) are massive performance bottlenecks.
**Action:** Replace iterative time series generation with vectorized NumPy operations. Specifically, build an array of step multipliers and use `np.cumprod` to generate the series, applying boundary conditions like `np.maximum` at the end rather than inside a loop.
