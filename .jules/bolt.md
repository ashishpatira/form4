...
## 2024-05-15 - Vectorizing Time Series Simulations
**Learning:** Standard Python `for` loops act as severe performance bottlenecks in time series simulations and numerical arrays.
**Action:** Replace `for` loops with vectorized NumPy operations, specifically leveraging `np.cumprod` and `np.maximum`.
