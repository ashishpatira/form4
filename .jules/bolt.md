...
## 2026-04-03 - Vectorizing Timeseries Simulations with Absorbing Bounds
**Learning:** When vectorizing timeseries simulation loops that enforce absorbing bounds (like flooring prices to 0 upon bankruptcy), using a final `np.maximum` after `np.cumprod` introduces a mathematical regression because future values shouldn't rebound.
**Action:** Identify the first boundary breach (e.g., `is_negative = multipliers <= 0`) and zero out all subsequent multipliers before running `np.cumprod` to ensure correctness while achieving significant performance gains (~20x faster).
