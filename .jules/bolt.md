...
## 2026-05-10 - Vectorizing Time Series Simulation
**Learning:** Python `for` loops for calculating time series (like arithmetic Brownian motion) are incredibly slow compared to vectorized NumPy operations.
**Action:** Always replace standard iterative simulation loops with highly optimized NumPy methods like `np.cumprod` and `np.maximum`, which can result in ~7-10x speedups.
