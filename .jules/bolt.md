...
## 2026-04-03 - Vectorizing Time Series Simulations
**Learning:** Python `for` loops are severe performance bottlenecks for time series simulations (e.g., arithmetic Brownian motion).
**Action:** Replace `for` loops with vectorized NumPy operations, leveraging `np.cumprod()` to calculate cumulative products and `np.maximum(prices, 0)` for functional zero-floor checks. This eliminates interpreter overhead and utilizes highly optimized C operations.
