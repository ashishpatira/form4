...
**Learning:** Standard Python `for` loops act as severe performance bottlenecks for time series simulations and numerical arrays.
**Action:** Replace standard loops with vectorized NumPy operations (e.g., `np.cumprod` and `np.maximum(prices, 0)`) for significant performance gains, ensuring any dead code like pre-allocated arrays (`np.zeros`) is removed.
