...
## 2026-04-03 - Vectorizing Timeseries with Absorbing Boundaries
**Learning:** When vectorizing timeseries simulation loops that enforce absorbing bounds (like flooring prices to 0 upon bankruptcy), applying a final `np.maximum(0, prices)` after `np.cumprod` introduces a mathematical regression because the entire timeseries bounces back up instead of staying dead at 0.
**Action:** Instead, identify the first boundary breach (e.g., `is_negative = multipliers <= 0`) and zero out all subsequent multipliers *before* running `np.cumprod` to ensure correctness while achieving significant performance gains (~95% speedup vs Python loops).
