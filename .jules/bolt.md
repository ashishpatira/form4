...
## 2026-04-03 - Numpy Vectorization over Python loops
**Learning:** Iterating over numpy arrays using a Python `for` loop (e.g., iteratively calculating prices in a timeseries) introduces massive interpreter overhead and defeats the purpose of C-optimized numpy routines.
**Action:** Replace sequential mathematical operations with vectorized numpy functions like `np.cumprod` to compute array modifications natively, resulting in ~40x performance improvements.
