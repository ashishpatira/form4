...
## 2026-04-03 - Vectorization for Time Series Simulations
**Learning:** In numerical Python applications, especially time series simulations over large arrays, standard Python `for` loops act as severe performance bottlenecks. Applying functions iteratively in a loop is much slower than utilizing optimized C operations.
**Action:** Always replace Python-level iteration over arrays with vectorized operations like `np.cumprod` for running products, and `np.maximum(prices, 0)` for element-wise conditional boundaries (like ensuring values don't go below 0).
