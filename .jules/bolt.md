...
## 2026-04-03 - Vectorizing Time Series Simulation
**Learning:** Standard Python `for` loops used for sequential calculations (like `prices[t] = prices[t-1] * factor`) over large arrays act as a severe performance bottleneck.
**Action:** Replace iterative loops with vectorized NumPy operations, specifically leveraging `np.cumprod()` (after prepending a starting value) and `np.maximum()`, to achieve order-of-magnitude performance gains for numerical simulations.
